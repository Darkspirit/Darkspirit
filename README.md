...
* Please be green on [hardenize.com](https://hardenize.com) and use IPv6 (against IPv4 fragmentation attacks), [RPKI](https://nlnetlabs.nl/projects/rpki/routinator/) for BGP, [ECDSA P-384 DNSSEC](https://www.cloudflare.com/dns/dnssec/ecdsa-and-dnssec/) ([RFC9824](https://datatracker.ietf.org/doc/html/rfc9824): [black lies](https://blog.cloudflare.com/black-lies/) instead of classic NSEC/[NSEC3](https://developers.cloudflare.com/dns/dnssec/enable-nsec3/)), TLSA/DANE, TLS 1.3, DMARC (`p=reject`), SPF (`v=spf1 mx -all` or: `TXT v=spf1 -all`+`MX 0 .`) for each (sub)domain, [CSP](https://report-uri.com/home/generate), [XHTML5](https://blog.whatwg.org/xhtml5-in-a-nutshell),
  ...
  * | Criterion | HTTP/1.1 (h1) | HTTP/2 (h2) | HTTP/3 (h3) |
    ...
    | Implementation Language | 🟢 **Safe Context** (Simple parsing reduces language-level risk) | 🔴 **Unsafe** (Client and server logic predominantly bound to memory-unsafe C/C++ engines like nghttp2) | 🟡 Hybrid (Firefox uses Rust; but Nginx still uses C) |
    ...
    | Infrastructure Resilience | 🟡 **Moderate** (Predictable under load but vulnerable to OS-level Socket-Exhaustion due to lack of multiplexing, and prone to slow byte-by-byte header transmission / Slowloris attacks) | 🔴 **Critical** (Vulnerable to protocol-induced CPU/RAM exhaustion; high attacker leverage; dead sockets block OS kernel) | 🟡 Conditional (Resilient to CPU/RAM exhaustion **ONLY IF** QUIC Retry Tokens are enforced (non-default config); lower attacker leverage due to rapid stateless resets) |
* Hardware
